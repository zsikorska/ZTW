...
function naph_admin_actions_register_menu(){
    add_options_page("Advertisment Publisher", "New Post Highliter", 'manage_options', "naph", "naph_admin_page");
}
...
function naph_mark_new_post_title($content, $id){
    ...
    $date = new DateTime(get_the_date('Ymd', $id));
    //get current date
    $now =  new DateTime(date('Ymd'));
    //get setting for how long post is a new post
    $opDays = get_option('naph_days');//generate proper post title
    if($date->diff($now)->format("%r%a") <= $opDays)
